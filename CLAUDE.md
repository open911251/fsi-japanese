...
**目前進度**（2026-08-10 更新）：**全50課骨架已補完**（過程見ROADMAP #22）。**代換詞加厚進行中**：實測發現「第1-13課有4-8個代換詞」的舊說法不準確，實際上只有第1-3課豐富，第4-50課（約68組句型）大多只有4個代換詞的最低量，使用者確認範圍是**第4-50課全部**，目標加厚到8個左右。第一批（第4-9課，19組句型）已完成並經獨立審查（抓到4處真撞句/矛盾如「新幹線去車站」語感矛盾、「寒くない」跟base重複，已修正）。**過程中發現並修好一個測試盲點**：`tests/test-shuffle.js`原本假設cue數不會超過`subCapInput`預設上限4（因為之前所有課次剛好都≤4），加厚後第8課cue數超過上限，暴露出測試沒考慮`subCapMap()`會截斷的邏輯，已改用`subCapMap()`回傳值當比對基準修正（不是內容錯誤，是測試假設過時）。舊28課的內容擴充進度（重整前）：`sub` 句型缺口第3-13課已補完（全教材句型數41→52、代換詞226個），第14課（現對齊新第14課）以後的舊課次還沒補；`trans` 轉換操練第1、2課已完成（新第1、2課），其餘還沒做。**接下來擴充要優先處理pending骨架課次**（照上面五步走，一次只挑1-3個pending課次，不要一次衝完全部剩下的20個）。
