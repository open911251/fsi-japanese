...
**目前進度**（2026-08-10 更新）：**全50課骨架已全數補完，pending課次數0**。第46、47課曾發現原規劃文法點跟第28、43課既有內容重複（見ROADMAP #22詳細記錄），使用者選擇「維持50課全部對齊真實MNN課號」（不引入中級教材的おかげで/せいで這種跨書內容），改成把第28課（只留ば〜ほど／ながら／のに／ことにする）、第43課（只留樣態そう）瘦身、抽出的內容還給46課（ところです／たばかり／はず）、47課（そうです伝聞／ようです），28、43課抽掉的量用新句子補回。全部經獨立agent兩輪審查（草稿＋抽換後複審）。**接下來要做的是代換詞加厚**（第14課以後大多只有4個代換詞的最低骨架量，第1-13課則有4-8個），使用者明確要求記住這件事，不要忘記啟動。舊28課的內容擴充進度（重整前）：`sub` 句型缺口第3-13課已補完（全教材句型數41→52、代換詞226個），第14課（現對齊新第14課）以後的舊課次還沒補；`trans` 轉換操練第1、2課已完成（新第1、2課），其餘還沒做。**接下來擴充要優先處理pending骨架課次**（照上面五步走，一次只挑1-3個pending課次，不要一次衝完全部剩下的20個）。
