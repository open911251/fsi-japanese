...
18. ⬜ 重音對立批量管線（kanjium）：優先度降低——第 15 項 B 子功能已用「任一詞查 VOICEVOX＋改寫 accent 合成錯誤版」達成類似訓練效果，不再依賴人工找同音詞對。若仍要做傳統「同音不同重音」詞義對立組（MINPAIRS 風格），流程不變：從 kanjium `accents.txt`（Yomichan 音調資料，約 12 萬詞條）按讀音分組撈詞對 → LLM 只填中文釋義 → 現在可直接用 VOICEVOX 指定重音合成，免 TTS 驗收這一步
