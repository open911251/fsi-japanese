...
17. ⬜ 腔調學習履歷（2026-07-19 使用者提案）：把 moraScore 的逐拍結果按重音型分類累積（平板/頭高/中高/尾高 各記 n/ok，加三個診斷錯誤型：核後該降沒降、平板被加下降、首拍該低讀高；按日分桶存 localStorage 保留 30 天）。回饋兩路——①進步偵測：近 7 天 vs 之前的正確率比較，顯著進步在回合結算/今日條稱讚（腔調進展慢，可視化進步對維持動機關鍵）；②弱點導向：最低正確率類型顯示於⑤，並餵給⑥重音位置聽辨抽題加權（同 mpPick 邏輯）。前置：vvMoras 需多帶各 phrase 的 accent 核位置；**先確認使用者實際體感 moraScore 判定夠準再做**（比例對齊誤判多的話先上 DTW 對齊，否則統計會被污染）
