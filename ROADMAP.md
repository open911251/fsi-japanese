...
7. ✅ 回合評分與課程順序（2026-07-13 完成）：回饋開啟的 sub/qa 每輪結算平均分（roundVerdict，≥75 合格）；合格建議下一課、未達建議重練（不強制鎖課）；每課 best/last 存 `fsi_score`，課程選單顯示徽章（取 sub/qa 較弱一項當精熟下限）。遲到的 STT 結果會補進結算
