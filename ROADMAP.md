...
12. ⬜ 卡拉OK式即時跟唱（提案中，使用者考慮中）：解「腔調矯正不及時」——示範曲線先畫上畫布當軌道，使用者開口的同時以 Web Audio（AnalyserNode 逐幀自相關，延遲約 50ms）即時畫出自己的音高，邊說邊修。全瀏覽器本地、不需後端。與現有事後對比互補：跟唱練即時控制、區段重聽練細節模仿、聽辨練知覺。中型功能：即時音高追蹤＋時間軸對齊（游標依示範時長掃過）＋跟唱 UI
